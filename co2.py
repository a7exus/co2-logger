...
class Co2Meter(object):
    """docstring for Co2Meter"""

    def __init__(self):
        if os.path.exists('/dev/tty.usbserial-20131207200'):
            serial_port='/dev/tty.usbserial-20131207200'
        elif os.path.exists('/dev/tty.SLAB_USBtoUART'):
    ...
    def setDate(self):
        t=str(int(time.time()-self.t_zero)).encode()
        self.ser.write(b'C '+t+b'\r')
...
with open('config.yaml', 'r') as file:
    influx_db_params = yaml.safe_load(file)
influx_db_url = influx_db_params['url'] 
...
while True:
    ...
    s = s.decode()
    try:
        data = re.search(r': T(?P<temp>\d\d\.\d)C:C(?P<co2>\d{3,4})ppm:H(?P<humid>\d\d\.\d)%', s).groupdict()
    ...
    try:
        r=requests.post(influx_db_url, linedata)
